...
def users_show(user_id):
    ...
    messages = (Message
                ...
                .limit(100)
                .all())
    return render_template('users/show.html', user=user, messages=messages, logged_in_user=logged_in_user)
...
def homepage():
    ...
    if user:
        messages = (Message
                    .query
                    .filter(Message.user_id.in_([following.id for following in user.following] + [user.id]))
                    .order_by(Message.timestamp.desc())
                    .limit(100)
                    .all())
